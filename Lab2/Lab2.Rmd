...
> The code sets the working directory and loads a CSV file
> "merged_data_lab2.csv" into a dataframe df. It converts the
> date-related columns (patent_date, filing_date, and year) to date
> format, ensuring the year column retains only the year part. The
> columns disambig_country and disambig_city are renamed to country and
> city, respectively. Next, it reads the "total_population.csv" file,
> adjusts its column names by replacing periods with underscores and
> removing leading 'X' characters. Similarly, it processes and renames
> columns in a CSV file containing country codes. Finally, the first
> five rows of the df, total_population, and country_codes dataframes
> are displayed to confirm correct data loading and initial processing.
...
# For each patent keep only the record with the first inventor (`inventor_sequence`=0).
df_filtered_1b <- df %>% filter(inventor_sequence == 0)

# Step 2: Sort the data by filing_date to get the newest and oldest patents
df_sorted_1b <- df_filtered_1b %>% arrange(filing_date)

# Display the following columns: `patent_id`, `filing_date`, `patent_title`, `Country`.
selected_columns_1b <- df_sorted_1b %>% select(patent_id, filing_date, patent_title, country)

# Display in a new table the five newest and five oldest patent requests.
newest_patents_1b <- selected_columns_1b %>% tail(5)
oldest_patents_1b <- selected_columns_1b %>% head(5)

# Combine the newest and oldest patents into a single table
result_1b <- bind_rows(oldest_patents_1b, newest_patents_1b)
...
# Step 1: Group the data by city and count the number of patents
city_patent_counts <- df %>% 
  group_by(city) %>% 
  summarise(patent_count = n()) %>% 
  arrange(desc(patent_count))

# Step 2: Select the top five cities with the most patents
top_five_cities <- city_patent_counts %>% head(5)
